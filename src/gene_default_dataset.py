...
class GeneRNADataset(Dataset):
    def __init__(self, fn):
        ...
        self.miRNA_vec = torch.from_numpy(data[:, :128].astype(np.float64))
        self.Gene_vec = torch.from_numpy(data[:, 128:256].astype(np.float64))
        self.label = torch.from_numpy(data[:, 258:259].astype(np.int_))
